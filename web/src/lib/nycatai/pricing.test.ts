...
describe("nycatai pricing", () => {
    ...
    it("受管渠道无价格数据的模型返回 null（宁缺毋滥）", () => {
        expect(estimateImageCost("nycatai-image::gpt-image-2", 1)).toBeNull();
    });

    it("按张计价 × 数量", () => {
        const estimate = estimateImageCost("nycatai-image::nano-banana-2", 3);
        expect(estimate).not.toBeNull();
        expect(estimate!.amount).toBeCloseTo(0.075 * 3);
    ...
    it("按秒计价 × 时长，非法数量兜底为 1", () => {
        ...
        expect(estimateVideoCost("nycatai-video::kling-3.0-720p", 0)!.amount).toBeCloseTo(0.08);
    });

    it("视频模型无价格返回 null", () => {
        expect(estimateVideoCost("nycatai-video::veo31-ref", 6)).toBeNull();
    });
