...
describe("nycatai pricing", () => {
    ...
    it("按张计价 × 数量", () => {
        const estimate = estimateImageCost("nycatai-image::nano-banana-2-1k", 3);
    ...
    it("按秒计价 × 时长，非法数量兜底为 1", () => {
        // 260826 三次定价：max(号池积分价 0.09, miku 候选 0.10×1.2) ⇒ ¥0.12（保多源）
        const estimate = estimateVideoCost("nycatai-video::kling-3.0-720p", 6);
        expect(estimate!.amount).toBeCloseTo(0.72);
        expect(estimateVideoCost("nycatai-video::kling-3.0-720p", 0)!.amount).toBeCloseTo(0.12);
    });
    ...
    it("单价文案按计费单位区分张/秒/次", () => {
        expect(unitPriceLabel("nano-banana-2-1k")).toBe("¥0.080/张");
        expect(unitPriceLabel("kling-3.0-720p")).toBe("¥0.12/秒");
    ...
    it("计费规则含单位说明、限制与线路风险", () => {
        expect(billingRuleLabel("kling-3.0-720p")).toContain("按秒计费");
